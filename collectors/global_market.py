    ...
    for attempt in range(1, FRED_MAX_RETRIES + 1):
        try:
            ...
            if response.status_code == 429:
                ...
                last_error = ValueError(f"FRED HTTP 429 (Retry-After={retry_after}s)")
                if attempt < FRED_MAX_RETRIES:
                    time.sleep(retry_after)
                continue
            if response.status_code >= 500:
                last_error = ValueError(f"FRED HTTP {response.status_code} 서버 오류")
                if attempt < FRED_MAX_RETRIES:
                    time.sleep((2.0 ** attempt) * random.uniform(0.75, 1.25))
                continue
            ...
            last_error = exc
            if attempt < FRED_MAX_RETRIES:
                time.sleep((2.0 ** (attempt - 1)) * random.uniform(0.75, 1.25))
    if text is None:
        raise ValueError(f"FRED batch 실패 ({FRED_MAX_RETRIES}회 시도): {last_error}")
