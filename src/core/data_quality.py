"""Data Quality Gate — raw collector output validation, API failure tracking,
and Point-in-Time (PiT) 공표시차 경고.
...
try:
    from src.core.pit_database import get_lag_info
    _PIT_AVAILABLE = True
except ImportError:
    _PIT_AVAILABLE = False
    def get_lag_info(k):  # type: ignore
        return {}
...
def validate_raw_ecos(
    ...
    월별 시리즈는 공표 시차(lag) 정보를 warnings에 포함한다.
    """
    results: dict[str, Any] = {}
    ...
    for key, rows in ecos_data.items():
        ...
        r = validate_ecos_series(name=key, rows=rows, prev_rows=prev_rows)

        # PiT 시차 경고: 월별 시리즈 감지
        if _PIT_AVAILABLE:
            lag_info = get_lag_info(key)
            if lag_info.get("frequency") == "monthly" and lag_info.get("lag_days", 0) >= 30:
                pit_warnings.append(
                    f"{key}: 월별 지표 공표시차 약 {lag_info['lag_days']}일 "
                    f"({lag_info.get('source', '')}). 백테스트 시 PiT 필터 적용 필요."
                )
